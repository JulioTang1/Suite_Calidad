...
<script runat="server">
    ...
    void Application_AcquireRequestState(object sender, EventArgs e)
    {
        ...
        if (Context.Request.Url.LocalPath != "/token.asmx/Validar_token" &&
            ...
            Context.Request.Url.LocalPath != "/Sincronizacion.asmx/Autenticar" &&
            Context.Request.Url.LocalPath != "/apiServices/Sincronizacion.asmx/Autenticar" &&
            Context.Request.Url.LocalPath != "/SERVER/Sincronizacion.asmx/Autenticar")
        {

            if (Context.Session != null){
                HttpSessionState session = Context.Session;
                if (session["Email"] == null && session["Token"] != null)
                {
                    //condiciones creadas para que la APP MOVIL pueda entrar a consumir servicios
                }
