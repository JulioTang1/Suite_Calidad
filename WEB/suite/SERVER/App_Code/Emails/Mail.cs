...
public class Mail
{
    ...
    public static void SendEmailNewUser(string nombre, string email)
    {
        String mail;
        String password;
        String nombreRemitente;
        ...
        adapter = new SqlDataAdapter(String.Format(@"
            ...
            INNER JOIN {0}.rol R ON EU.id_rol = R.id
            INNER JOIN {0}.rol_aplicacion RA ON R.id = RA.id_rol
            INNER JOIN {0}.aplicacion APP ON RA.id_app = APP.id
            AND APP.id = 16
            AND RA.id_nivel_acceso = 2
            INNER JOIN {0}.estado_opcion EO ON EU.id_estado_opcion = EO.id
            AND EO.id = 1
            INNER JOIN {0}.permisos_rol_menu P ON P.id_rol = R.id
            AND APP.id = P.id_aplicacion
            INNER JOIN {0}.menu MENU ON P.id_menu = MENU.id
            AND MENU.edit_state = 1
            INNER JOIN {0}.nivel_acceso NA ON P.id_nivel_acceso = NA.id
            AND NA.id = 2
        ...
        for(int x = 0; x < dt.Rows.Count; x++)
        {
            ...
            stringBuilder.Append("Solicitud de Acceso - Banasan");
            ...
            stringBuilder.Append("Hola " + row["nombre"] + ",<br><br>El siguiente usuario ha solicitado acceso.<br><br>");
            stringBuilder.Append("- " + nombre + "<br>");
            stringBuilder.Append("- " + email + "<br><br>");
            stringBuilder.Append("Por favor ingresa al módulo de gestión de usuarios para activarlo y asignarle un rol.<br><br>Muchas gracias.");
            stringBuilder.Append("</span>");
            stringBuilder.Append("</div>");
            stringBuilder.Append("</div>");
